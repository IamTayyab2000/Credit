...
function getBillSummaries($conn, $customer_id) {
    ...
    // The view should expose: customer_id, bill_id, event_date, src_ledger_id, ref_id,
    // debit, credit, source_type, bill_status, running_balance
        // Group view events by bill to produce a single row per bill (document-level)
        $query = "SELECT 
                                bill_id,
                                MIN(event_date) AS ledger_date,
                                GROUP_CONCAT(DISTINCT ref_id ORDER BY event_date SEPARATOR ', ') AS ref_ids,
                                SUM(debit) AS bill_amount,
                                SUM(credit) AS recived_amount,
                                (SUM(debit) - SUM(credit)) AS remaining_amount,
                                COALESCE(MAX(bill_status),'') AS bill_status
                            FROM vw_customer_bill_ledger
                            WHERE customer_id = ?
                            GROUP BY bill_id
                            ORDER BY ledger_date ASC, bill_id ASC";
...
function getRawTransactions($conn, $customer_id) {
    $query = "SELECT ledger_id, ref_id, ledger_date, bill_id, bill_amount, recived_amount, remaining_amount, COALESCE(bill_status,'') AS bill_status
              FROM bill_ledger
              WHERE customer_id = ?
...
function getOpeningBalance($conn, $customer_id, $from_date = null) {
    if ($from_date !== null) {
        $query = "SELECT COALESCE(SUM(bill_amount - recived_amount),0) AS opening FROM bill_ledger WHERE customer_id = ? AND ledger_date < ?";
        if ($stmt = $conn->prepare($query)) {
...
// Fetch and render bill summaries (document-level view)
...
$bill_rows = getBillSummaries($conn, $customer_id);

$base_q = '?cust_id=' . urlencode($customer_id);
if ($from) $base_q .= '&from=' . urlencode($from);
echo '<p><a href="' . $base_q . '">Summary</a> | <a href="' . $base_q . '&show_raw=1">Raw Entries</a></p>';
...
if (empty($bill_rows)) {
    ...
    foreach ($bill_rows as $row) {
        ...
        echo '<td>' . h($row['ref_ids']) . '</td>';
        echo '<td>' . h($row['ledger_date']) . '</td>';
        echo '<td style="text-align:right">' . number_format($debit,2) . '</td>';
        echo '<td style="text-align:right">' . number_format($credit,2) . '</td>';
        echo '<td style="text-align:right">' . number_format($remaining,2) . '</td>';
        echo '<td style="text-align:right">' . number_format($running,2) . '</td>';
    ...
    echo '<td style="text-align:right">' . number_format($total_remaining,2) . '</td>';
    echo '<td style="text-align:right">' . number_format($running,2) . '</td>';
    echo '<td></td>';
    echo '</tr>';
    echo '</tfoot></table>';
